...
KishorgonjApp = Flask(__name__)

@KishorgonjApp.route('/')
def index():
    return render_template('1Cus3.4_Ticket4Kishorgonj.html')

@KishorgonjApp.route('/KishorgonjResult', methods=['POST', 'GET'])
def result():
    if request.method == 'POST':
        ...
        if number_of_seats > available:
            ...
            cursor.execute("INSERT INTO reservation (total_fare, no_of_tickets, train_no, customer_id, location_name) VALUES (?, ?, ?, ?, 'Kishorgonj')", (fare, number_of_seats, train_number, customer_id))
