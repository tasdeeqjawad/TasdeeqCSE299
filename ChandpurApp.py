...
ChandpurApp = Flask(__name__)

@ChandpurApp.route('/')
def index():
    return render_template('1Cus3.9_Ticket4Chandpur.html')

@ChandpurApp.route('/ChandpurResult', methods=['POST', 'GET'])
def result():
    if request.method == 'POST':
        ...
        if number_of_seats > available:
            ...
            cursor.execute("INSERT INTO reservation (total_fare, no_of_tickets, train_no, customer_id, location_name) VALUES (?, ?, ?, ?, 'Chandpur')", (fare, number_of_seats, train_number, customer_id))
