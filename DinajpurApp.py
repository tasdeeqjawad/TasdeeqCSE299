...
DinajpurApp = Flask(__name__)

@DinajpurApp.route('/')
def index():
    return render_template('1Cus3.7_Ticket4Dinajpur.html')

@DinajpurApp.route('/DinajpurResult', methods=['POST', 'GET'])
def result():
    if request.method == 'POST':
        ...
        if number_of_seats > available:
            ...
            cursor.execute("INSERT INTO reservation (total_fare, no_of_tickets, train_no, customer_id, location_name) VALUES (?, ?, ?, ?, 'Dinajpur')", (fare, number_of_seats, train_number, customer_id))
