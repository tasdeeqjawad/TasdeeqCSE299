...
SylhetApp = Flask(__name__)

@SylhetApp.route('/')
def index():
    return render_template('1Cus3.11_Ticket4Sylhet.html')

@SylhetApp.route('/SylhetResult', methods=['POST', 'GET'])
def result():
    if request.method == 'POST':
        ...
        if number_of_seats > available:
            ...
            cursor.execute("INSERT INTO reservation (total_fare, no_of_tickets, train_no, customer_id, location_name) VALUES (?, ?, ?, ?, 'Sylhet')", (fare, number_of_seats, train_number, customer_id))
