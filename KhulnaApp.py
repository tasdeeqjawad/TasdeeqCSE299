...
KhulnaApp = Flask(__name__)

@KhulnaApp.route('/')
def index():
    return render_template('1Cus31Cus3.6_Ticket4Khulna.html')

@KhulnaApp.route('/KhulnaResult', methods=['POST', 'GET'])
def result():
    if request.method == 'POST':
        ...
        if number_of_seats > available:
            ...
            cursor.execute("INSERT INTO reservation (total_fare, no_of_tickets, train_no, customer_id, location_name) VALUES (?, ?, ?, ?, 'Khulna')", (fare, number_of_seats, train_number, customer_id))
